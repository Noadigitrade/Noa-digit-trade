...
const CONFIG = {
  ...
  // Spread appliqué sur le taux du jour, pour chaque réseau :
  // achat = taux x (1 + spread), vente = taux x (1 - spread)
  networkSpreadPercent: 0.005,
  ...
  depositAddresses: {
    ...
    // TODO : remplacer par l'adresse Plasma réelle du portefeuille NOA/FASO
    plasma:
      '',
    ...
    // TODO : renseigner l'ID à utiliser pour les transferts internes "ID"
    id:
      ''
  ...
  if (!address) {
    ...
    if (
      ...
      return {
        ...
        message:
          network === 'uid'
            ? 'Veuillez saisir votre UID Binance.'
  ...
  if (
    ...
    if (
      ...
      return {
        ...
        message:
          network === 'uid'
            ? "Cet UID Binance ne semble pas valide (chiffres uniquement, sans espace)."
            : "Cet ID ne semble pas valide."
      };
    }
  ...
  if (walletHelp) {
    ...
    walletHelp.textContent =
      onChain
        ...
        : currentNetwork === 'uid'
          ? "⚠️ Indiquez votre UID Binance : vos USDT seront envoyés par transfert interne (Pay), sans frais réseau."
          : "⚠️ Indiquez votre ID : vos USDT seront envoyés par transfert interne, sans frais réseau.";
  }
}
...
function calculateOrder() {
  ...
  if (
    ...
    // Spread : le client paie plus cher à l'achat (taux x (1 + spread))
   ...
   * Spread : le client reçoit moins à la vente (taux x (1 - spread))
   */
